...
You can also asynchronously output messages with Commander.output('message') """
    ...
    PALLETE=[('reversed', urwid.BLACK, urwid.LIGHT_GRAY),
              ...
              ('buy', urwid.WHITE, urwid.DARK_GREEN),
              ('neutral', urwid.BLACK, urwid.LIGHT_GRAY),
              ('sell',urwid.WHITE,urwid.DARK_RED),
    ...
    def updateTraderWidget(self, type, name, value):
        widgets = []
        if type == 'signal':
            widgets = self.signals
        elif type == 'indicator':
            widgets = self.indicators
        elif type == 'strategy':
            widgets = self.strategies
        for widget in widgets:
                splitted = widget.get_text()[0].split(" : ")
                if splitted[0] == name:
                    if isinstance(value, str):
                        widget.set_text(('neutral', splitted[0]+" : "+str(value)))
                    else:
                        if value==0:
                            color='neutral'
                        elif value>0:
                             ...
                             color='sell'
                        if type=='signal':
                            widget.set_text((color, splitted[0]))
                        else:
                            widget.set_text((color, splitted[0]+" : "+str(value)))
    ...
    def createTraderWidgets(self, type):
        ...
        if type=='signals':
            ...
            for element in Helper.getAvailableIndicators():
                temp.append(urwid.Text(('neutral', element+" : "+str(0.0))))
    ...
    def __init__(self, title, command_caption='Command:  (Tab to switch focus to upper frame, where you can scroll text)', cmd_cb=None, max_size=1000):
        ...
        self.signals = self.createTraderWidgets('signals')
        self.indicators = self.createTraderWidgets('indicators')
        self.strategies = self.createTraderWidgets('strategies')

        self.titleText=	urwid.Text(title)
        self.OHLC=urwid.Text('HERE COMES THE OHLC')
        self.indicatorsGrid=urwid.GridFlow(self.indicators,15,1,1,'left')
        self.signalsGrid=urwid.GridFlow(self.signals,15,1,1,'left')
        self.strategiesGrid=urwid.GridFlow(self.strategies,15,1,1,'left')

        self.header=urwid.Pile(
				[urwid.AttrMap(self.titleText,'normal'),
				urwid.AttrMap(self.OHLC,'normal'),
				urwid.AttrMap(urwid.Text("Indicators"),'normal'),
				self.indicatorsGrid,
                                urwid.AttrMap(urwid.Text("Signal"),'normal'),
				self.signalsGrid,
				urwid.AttrMap(urwid.Text("Strategies"),'normal'),
				self.strategiesGrid,
				])
    ...
    def updateOHLCText(self, string):
        self.OHLC.set_text(string)
        if self.eloop and self._eloop_thread != threading.current_thread():
            self.eloop.draw_screen()
